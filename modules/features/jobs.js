...
export async function analyseJob() {
  ...
  try {
    ...
    const profileCtx = (prof?.accessNeeds || prof?.values)
      ? `\n\nCANDIDATE PROFILE:\n${prof.accessNeeds ? `Access needs: ${prof.accessNeeds}\n` : ""}${prof.values ? `Work values: ${prof.values}\n` : ""}`
      : "";
    const valuesSchema = profileCtx ? `,"values_match":{"score":0,"reason":"1-2 sentence explanation of alignment or misalignment with candidate profile"}` : "";
